...
app.post("/Register", (req, res) => {
  ...
  dashDB.query(
    "INSERT INTO dashboarddb.users (username, password, buffer, email, photo) VALUES (?,?,?,?,?)",
    [
      username,
      encryptedPassword.password,
      encryptedPassword.initial,
      email,
      photo,
    ],
    (error, result) => {
...
app.post("/Login", (req, res) => {
  ...
  dashDB.query(
    "SELECT password, buffer FROM dashboarddb.users WHERE username = ?",
    [username],
    (error, result) => {
      if (error) {
        res.send({ error: error });
...
app.get("/Pokemon", (req, res) => {
  const url = "http://pokeapi.co/api/v2/pokemon/" + getRandomPokemonID();
  const pokemonOptions = {
    method: "GET",
    url: url,
  };
  Axios.request(pokemonOptions)
    .then((response) => {
  ...
  puppeteer
    ...
    .then((html) => {
      ...
      $("p").each(function () {
        ...
        let text = $(this).text();
        if (
          !["We use cookies", "Yes, I agree", "No, take me to settings"].some(
            (v) => text.includes(v)
          )
        ) {
          fullText += " " + text;
...
app.post("/RecentPhotos", (req, res) => {
  dashDB.query(
    "SELECT photolink FROM dashboarddb.photos WHERE user = ? order by created desc limit 5",
    [req.body.username],
    (error, result) => {
      if (error) {
        res.send({ error: error });
...
app.post("/DeleteImage", (req, res) => {
  dashDB.query(
    "DELETE FROM dashboarddb.photos WHERE user= ? AND photolink = ?",
...
app.post("/PostImageDB", (req, res) => {
  dashDB.query(
    "INSERT INTO dashboarddb.photos (user, photolink) VALUES (?,?)",
...
app.post("/PostTask", (req, res) => {
  dashDB.query(
    "INSERT INTO dashboarddb.tasks (status, description,user) VALUES (?,?,?)",
...
app.post("/UpdateTask", (req, res) => {
  dashDB.query(
    "UPDATE dashboarddb.tasks SET status = ?, description= ? WHERE taskid = ?",
...
app.post("/GetTasks", (req, res) => {
  dashDB.query(
    "SELECT taskid, description, status FROM dashboarddb.tasks WHERE user = ? order by taskid",
    [req.body.username],
    (error, result) => {
      if (error) {
        res.send({ error: error });
